...
// --- CEREBRO DEL BOT (AQUÍ ESTÁ LA ACTUALIZACIÓN ÚNICA) ---
const SalesTerminal = ({ context }) => {
  ...
  const handleSend = async (e) => {
    ...
    const textoAnalisis = input.toLowerCase().trim();
    const saludos = ["hola", "buenos dias", "buenas", "que tal", "inicio", "prueba"];

    setInput("");
    
    // Poner mensaje usuario en pantalla
    setMessages(prev => [...prev, { id: Date.now(), type: 'user', text: userText }]);
    setReportData(null); 
    
    // --- 2. FILTRO LOCAL "HOLA" (LA MAGIA) ---
    // Si dice Hola, responde el celular al instante.
    if (saludos.some(s => textoAnalisis.includes(s)) && textoAnalisis.length < 15) {
        setIsTyping(true);
        setTimeout(() => {
            setIsTyping(false);
        ...
        return; // ¡DETIENE EL PROCESO AQUÍ PARA NO IR AL SERVIDOR!
    }

    // --- 3. SI NO ES SALUDO, VAMOS AL SERVIDOR REAL ---
    setIsTyping(true);

    try {
        // Separamos nombre de ciudad si es posible
        const parts = userText.split(' en ');
        const businessName = parts[0];
        const city = parts[1] || "Mexico"; 
        ...
        if (!data.found) {
            ...
            // Caso: Encontrado (Muestra el reporte agresivo del servidor)
            setMessages(prev => [...prev, { 
                id: Date.now() + 1, 
                type: 'bot', 
        ...
        setMessages(prev => [...prev, { 
            id: Date.now() + 1, 
            type: 'bot', 
            text: "⚠️ Error de conexión: El servidor está despertando. Intente de nuevo en 30 segundos." 
        }]);
    }
  };
