...
const DEFAULT_EVENT: EventConfig = {
    ...
    coords: '',
    image: '',
    description: 'Te invitamos a nuestro informe de actividades. ¡Tu presencia es importante!',
    time: '6:00 PM',
}
...
export default function CitizenEventPage() {
    ...
    /* ---- RSVP: Confirm attendance + save contact ---- */
    const handleRSVPSubmit = async () => {
        ...
        try {
            ...
            setWhatsAppMessage(`✅ Confirmo mi asistencia\n📋 ${confirmedName}\n🏛️ ${event.name}`)
            setShowVCardPrompt(true)

            // Auto-download vCard (doble seguro)
        ...
        <div className="min-h-screen" style={{ background: '#0f1117', fontFamily: "'Inter', 'Helvetica Neue', sans-serif" }}>
            ...
            <section className="relative text-center overflow-hidden">
                ...
                <div className="px-4 py-5" style={{ background: 'rgba(255,255,255,0.02)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    <p className="text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: accent }}>⏳ Faltan</p>
            ...
            {contacts.length > 0 && (
                <section className="px-4 py-5" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    <p className="text-center text-sm mb-4" style={{ color: accent }}>
                        ✅ {contacts.length} {contacts.length === 1 ? 'persona confirmada' : 'personas confirmadas'}
                    </p>
            ...
            {showRSVP && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)' }}
                    ...
                    <div className="rounded-2xl p-5 w-full max-w-sm" style={{ background: '#1e1f2e', border: '1px solid rgba(255,255,255,0.1)' }}
                        ...
                        <h3 className="text-lg font-bold text-white mb-0.5">✅ Confirmar Asistencia</h3>
                        <p className="text-xs text-white/40 mb-5">Confirma tu llegada al evento</p>
            ...
            {/* ==========================================
                DOBLE SEGURO: VCARD + WHATSAPP MODAL
                ========================================== */}
            {showVCardPrompt && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)' }}
                    onClick={() => setShowVCardPrompt(false)}>
                    <div className="rounded-2xl p-6 w-full max-w-sm" style={{ background: '#1e1f2e', border: '1px solid rgba(255,255,255,0.1)' }}
                        onClick={(e) => e.stopPropagation()}>
                        <div className="text-center mb-5">
                            <div className="text-3xl mb-2">✅</div>
                            <h3 className="text-lg font-bold text-white">¡Asistencia confirmada!</h3>
                            <p className="text-xs text-white/40 mt-1">Completa estos dos pasos:</p>
                        </div>

                        {/* PASO 1: Guardar contacto */}
                        <div className="rounded-xl p-4 mb-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                            <div className="flex items-start gap-3">
                                <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-black text-white" style={{ background: accent }}>1</div>
                                <div className="flex-1">
                                    <p className="text-sm font-bold text-white mb-1">💾 Guarda el contacto</p>
                                    <p className="text-[0.65rem] text-white/40 mb-2.5">Se descargó un archivo de contacto. Ábrelo para guardar a <strong className="text-white/60">{config.name}</strong> en tu teléfono.</p>
                                    <button onClick={triggerVCardDownload}
                                        className="w-full py-2.5 rounded-lg text-xs font-bold text-white flex items-center justify-center gap-1.5 transition-all active:scale-95"
                                        style={{ background: accent }}>
                                        📥 Descargar de nuevo
                                    </button>
                                </div>
                        ...
                        <div className="rounded-xl p-4 mb-4" style={{ background: 'rgba(37,211,102,0.05)', border: '1px solid rgba(37,211,102,0.12)' }}>
                            <div className="flex items-start gap-3">
                                <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-black text-white" style={{ background: '#25d366' }}>2</div>
                                <div className="flex-1">
                                    <p className="text-sm font-bold text-white mb-1">📲 Envía un WhatsApp</p>
                                    <p className="text-[0.65rem] text-white/40 mb-2.5">Envía un mensaje para recibir info de próximos eventos.</p>
                                    <div className="rounded-lg p-2 mb-2.5 text-left" style={{ background: 'rgba(37,211,102,0.08)' }}>
                                        <p className="whitespace-pre-line text-white/60 text-[0.65rem]">{whatsAppMessage}</p>
                                    </div>
                        ...
                        <button onClick={() => setShowVCardPrompt(false)}
                            className="w-full py-2 rounded-xl text-[0.65rem] text-white/20 hover:text-white/35 transition-colors">
                            Cerrar
                        </button>
                    </div>
                </div>
