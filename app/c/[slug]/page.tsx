            ...
            {showVCardPrompt && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)' }}
                    ...
                    <div className="rounded-2xl p-6 w-full max-w-sm text-center" style={{ background: '#1e1f2e', border: '1px solid rgba(255,255,255,0.1)' }}
                        ...
                        <h3 className="text-lg font-bold text-white mb-2">¡Último paso!</h3>
                        <p className="text-sm text-white/50 mb-2">
                            Envía un mensaje por WhatsApp a <strong className="text-white/80">{config.name}</strong> para completar tu confirmación.
                        </p>
                        <p className="text-xs text-white/30 mb-5">
                            Así podrás recibir información de próximos eventos.
                        </p>

                        {/* Preview del mensaje */}
                        <div className="rounded-xl p-3 mb-5 text-left text-sm" style={{ background: 'rgba(37,211,102,0.08)', border: '1px solid rgba(37,211,102,0.15)' }}>
                            <p className="whitespace-pre-line text-white/70 text-xs">{whatsAppMessage}</p>
                        </div>

                        <button onClick={openWhatsApp}
                            className="w-full py-3.5 rounded-xl text-sm font-bold text-white mb-3 transition-all active:scale-95 flex items-center justify-center gap-2"
                        ...
                        <div className="flex gap-2">
                            <button onClick={triggerVCardDownload}
                                className="flex-1 py-2 rounded-xl text-[0.65rem] text-white/25 hover:text-white/40 transition-colors"
