...
export default function DashboardPage() {
    ...
    const [config, setConfig] = useState<PoliticianConfig>({
        ...
        photo: '', dashboardPassword: 'admin123', accentColor: '#8B1A2B', backgroundColor: '#1a0f14', textColor: '#FFFFFF',
    })
    ...
    /* ================================================================
       ...
       ================================================================ */
    return (
        <div className="min-h-screen tpc" style={{ background: bgColor, color: textColor }}>
            ...
            {activeTab === 'contacts' && (
                <div className="px-4 pb-8">
                    ...
                    <div className="flex gap-2 mb-4">
                        <button onClick={() => exportContacts('google')} data-btn
                            className="px-4 py-2 rounded-xl text-xs font-bold text-white flex items-center gap-1.5"
                            style={{ background: '#34a853' }}>
                            📥 Google Contacts
                        </button>
                        <button onClick={() => exportContacts('excel')} data-btn
                            className="px-4 py-2 rounded-xl text-xs font-bold text-white flex items-center gap-1.5"
                                        ...
                                        <tr key={c.id} className="border-t" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
                                            ...
                                            <td className="px-3 py-2.5">
                                                <div className="flex items-center gap-1.5">
                                                    <a href={`https://wa.me/52${c.phone}`} target="_blank" rel="noopener noreferrer" data-btn
                    ...
                    <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                { key: 'textColor', label: 'Color de texto', placeholder: '#FFFFFF' },
                                { key: 'dashboardPassword', label: 'Contraseña del dashboard', placeholder: '...' },
                            ].map(field => (
                                <div key={field.key}>
                                    <label className="text-[0.6rem] text-white/30 font-bold uppercase tracking-wider mb-0.5 block">{field.label}</label>
                                    <input type={field.key === 'dashboardPassword' ? 'password' : (field.key === 'accentColor' || field.key === 'backgroundColor' || field.key === 'textColor') ? 'color' : 'text'}
                        ...
                        <div className="flex gap-2 mt-5">
                            {isEditingConfig ? (
                                <>
                                    ...
                                    <button onClick={saveConfig} disabled={isSaving} data-btn
                                        className="flex-1 py-3 rounded-xl text-xs font-bold text-white disabled:opacity-30"
                                        style={{ background: accent }}>
